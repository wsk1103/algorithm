...
public class L8 {
     ...
     * //
     * // Related Topics 字符串 👍 1457 👎 0
     */

    /**
     * 执行耗时:2 ms,击败了29.00% 的Java用户
     * 内存消耗:41.3 MB,击败了50.22% 的Java用户
     *
     * @param s
     * @return
     */
    public static int handle(String s) {
        ...
        if (sb.length() > mm.length()) {
            if (isStart) {
                return Integer.MIN_VALUE;
            }
            return Integer.MAX_VALUE;
        }
        if (sb.length() == mm.length() && sb.charAt(0) == '2') {
            for (int i = 2; i < sb.length(); i++) {
                if (sb.charAt(i - 1) == mm.charAt(i - 1) && sb.charAt(i) > mm.charAt(i)) {
                    if (isStart) {
                        return Integer.MIN_VALUE;
                    }
                    return Integer.MAX_VALUE;
        ...
        } else if (sb.length() == mm.length() && sb.charAt(0) > '2') {
            if (isStart) {
                return Integer.MIN_VALUE;
            }
            return Integer.MAX_VALUE;
